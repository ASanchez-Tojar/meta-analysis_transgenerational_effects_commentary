...
```{r, results="hide"}
# # re-obtaining dataframe listing the Open Tree identifiers potentially matching our list of species (be aware that this will take a few minutes, and you can simply load the tree and relationship matrix that we used below). The following code obtains the taxonomic data (which was obtained on the 14th of October 2019 for our study), does some checking of such data, retrieves the phylogenetic relationships, creates a relationship matrix, and makes sure than the dataset and the matrix have the same nomenclature. To speed up the process, the code is commented, and both the phylogenetic tree and its relationship matrix can be directly loaded below. Note that the Open Tree Taxonomy and Open Tree of Live are ongoing projects, and there might be some differences between our tree and relationship matrix, and trees and matrices built in a different point in time. However, we provide our tree and relationship matrices to make our process reproducible.
# 
# taxa.corrected <- tnrs_match_names(names = unique(trans.final$species.2))
# 
# # saving the final, corrected taxonomic data created on the 14th of October 2019.
# save(taxa.corrected,file = "../data/taxa_Open_Tree_of_Life_corrected.RData")
# 
# # loading the taxonomic data created on the 14th of October 2019
...
load("../data/tree_random.Rdata") #tree_random
...
# we can then plot the tree, but notice that the species names shown in the tree are not exactly the same as the species names that we had in our list. This is because those names had synonyms in the tree of life database, and we are using those names for the plot. This tree uses the data once polytomies have been dealt with.
plot(tree_random, cex=1, label.offset =.1, no.margin = TRUE)
```
